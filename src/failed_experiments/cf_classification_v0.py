...
How to run:
   python ./src/cf_classification_v0.py \
       --dataset tee \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=0,
        choices=[0, 1],
        help="Whether to include raw time series JSON in the prompt.",
    )
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    prompter.system_prompt = (
        ...
        " You will be given a time series visualization, and an extensive description of each possible classification label."
        " Your job is to use the time series visualization and the class descriptions to answer the multiple choice question."
    ...
    for start_of_batch in tqdm(
        range(0, len(test), args.batch_size), desc="Processing batches"
    ):
        ...
        for row in batch:
            user_kwargs: Dict[str, Any] = {
                ...
                "class_descriptions": features_str,
            }
            if args.include_ts:
            ...
            img_path = plot_time_series(
                ...
                save_path=(
                    f"{test_img_dir}/idx_{int(np.asarray(row.idx).item())}_"
